...
namelist()
{
	...
	if (S(mag_un.mag_exp.a_magic) == OARMAG) {
		error(0, "old archive");
		goto out;
	}
	...
	do {
		...
		fseek(fi, o, 1);
		n = L(mag_un.mag_exp.a_syms) / sizeof(struct nlist);
		if (n == 0) {
		...
		if (0) printf("file at %d, want to read %d\n", (int)ftell(fi), (int)(strsiz-sizeof(strsiz)));
		if (fread(strp+sizeof(strsiz),strsiz-sizeof(strsiz),1,fi) != 1)
			error(1, "error reading string table");
		for (j = 0; j < i; j++) {
			if (0) printf("j %d, n_strx %d\n", j, L(symp[j].n_un.n_strx));
			if (L(symp[j].n_un.n_strx))
				symp[j].n_un.n_name =
					L(symp[j].n_un.n_strx) + strp;
			else
				symp[j].n_un.n_name = "";
		}
		if (pflg==0)
	...
	for (n=0; n<nsyms; n++) {
		...
		if (c & N_STAB) {
			...
			printf("%08x - %02x %04x %5.5s %s\n",
			       (unsigned int)L(symp[n].n_value),
			       symp[n].n_other & 0xff, S(symp[n].n_desc) & 0xffff,
		...
		if (!uflg) {
			if (c=='u' || c=='U')
				...
				printf(N_FORMAT, (unsigned int)L(symp[n].n_value));
			printf(" %c ", c);
		}
		printf("%s\n", symp[n].n_un.n_name);
l1:		;
