 ...
 *
 * Philip Almquist  May 17, 1985
 *	- Fix printout of bus and address errors by putting a pointer
 *	  to the global data (GlobPtr) in location 0.
 *
 * Philip Almquist  June 3, 1985
 *	- experimental fix: if TREWITT is defined, update memory size in
 *	  GLOBRAM structure on autoreboot in case it  was trashed by a the
 *	  user program
 *
 * Philip Almquist  Novermber 4, 1985
 *	- Use defined constants instead of literal digits in version
 *	  number (fix from Ross Finlayson)
 *
 * Philip Almquist  January 7, 1986
 *	- Remove previous edit; it confuses the assembler
 *	- Remove conditional around June 3, 1985 fix (it seems to work)
 *	- Merge sunmon.c and ssunmon.c into one module.  Previously,
 *	  ssunmon.c was divergent version of sunmon.c whose major feature
 *	  was working WATCHDOG code.
 */
...
/*	asm("reset");		/* issue a reset instruction */
	...
	ResetTimer;
    	TMRLoadCmd(TCLdDataPtr(TSMasterMode));
    	LoadReg(TMMBus16 + TMMFoutOff + TMMFoutDiv2);
...
/*temp*/ setmode('A');			/* to allow error I/O */
	...
	*(long*)GXBase = 0;	/* touch frame buffer */
	...
	/* Restore location 0 to correct value (needed by SAVEACCESS macro) */
	*(long*)0 = (long)gp;
