 ...
 *  	SMI 10 Mhz watchdog support    	    	    	22 Mar 1983
 */
...
LabelGbl(HardReset);		/* branch here for hard reset */
{
	...
	asm ("reset");		/* issue a reset instruction */
    	...
    	SETSEGMAP(0, 0x0F00);	/* set segment and page maps for page */
    	SETPAGEMAP(1,0xC001);	/* RAM at 0x800 - 0xFFF for stuff */

/*    	asm ("movw  #INITSPa,sp"); /* Reset stack ptr (for software resets) */
    	asm ("pea   _ExitOp");	   /* Return address for rts */
    	asm ("pea   USERCODE");	   /* fake PC = User code default addr */
    	asm ("movw  sr,sp@-");	   /* Save current SR (should be 2700) */
    	asm ("orw   #0x0700,sr);   /* Make sure interrupts are disabled */
    	ExcCause (EXC_RESET);	   /* Exception cause */
    	asm ("subql #4,sp");	   /* Make room on the stack for USP */
/*    	asm ("pea   INITSPa"); */
    	asm ("moveml	#0xFFFE,sp@-");	/* save all but SSP and USP */
    	asm ("movl  usp,a0");	    	/* Save USP */
    	asm ("movl  a0,sp@(_disp_us)");


    	TMRLoadCmd(TC16BitBus);	    /* Make sure the bus mode is correct */

/*      Reset Timers, to avoid being interrupted */
    	TMRLoadCmd(TCGateOff);
	TMRLoadCmd(TCClrOutput(TIMRefresh));	/* clear possible interrupt */
	TMRLoadCmd(TCClrOutput(TIMWatchDog));	/* clear possible interrupt */

    	TMRLoadCmd(TCLdDataPtr(TIMWatchDog));	/* Get the mode register */
    	EraseMem = 1;    
    	if (TModWatchDog2 == SunTimerReg)
    	    {
    	     SoftReset = GETPAGEMAP(0);
    	     SETPAGEMAP(0,0xC000);
    	     gp = GlobPtr;
    	     EXITBOOT;
    	     gp -> DogMap0 = SoftReset;
    	     gp -> DogRefr = *EVEC_LEVEL7;  	/* Save refresh pointer */
    	     SoftReset = gp -> WhyReset;
    	     asm("movw	d6,sp@(_disp_cause)");  /* assume soft reset in d6 */
    	     EraseMem = 0;

    	     if (SoftReset != EXC_DOG)
    	    	{
    	    	 /* if booting shut off the interrupt timer */
    	    	 TMRLoadCmd(TCDisarmCnt(TSCountSelect(TIMInter)));
    	    	 TMRLoadCmd(TCClrOutput(TSCountSelect(TIMInter)));
    	    	 goto RemapMem;
    	    	}
    	     }
...
#ifdef	WATCHDOG
	/* Refresh task is ready and waiting */
    	TMRLoadCmd(TCGateOff);
	TMRLoadCmd(TCClrOutput(TIMRefresh));	/* disarm temporarily */
...
/*temp*/ setmode('A');			/* to allow error I/O */
	...
	*(long*)GXBase = 0;	/* touch frame buffer */
	...
	excvmake(EVEC_BUSERR,BusError);	/* catch Bus Errors */
#endif	FRAMEBUF
#ifdef	FRAMEBUF
