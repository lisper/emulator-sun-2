 ...
 *					23-February-1981
 *
 * Philip Almquist September 28, 1984
 *	- Change PUP to ETHERNET_TYPE_PUP so it will compile under 4.2
 *	- Change roundup to roundup2 for the same reason (arggh)
 */
...
register int	buflen;				/* data buffer length */
{
	...
	roundlen = roundup2(buflen);	/* rounds UP to even number of bytes */
	...
	*((short *) &enpack.Pup.PupData[roundlen]) = NOCKSUM;
	...
	return(enwrite(Pchan->ImmHost,ETHERNET_TYPE_PUP,Pchan->ofid,
