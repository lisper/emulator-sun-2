...
/* Enable or disable function code emulation */
	...
	#endif
#define m68ki_remember_access(P, A, F, W, S) { m68ki_access_pc = (P); m68ki_access_address = (A); \
					       m68ki_access_fc = (F); m68ki_access_write = (W); \
					       m68ki_access_size = (S); }
	...
	#define m68ki_remember_access(P, A, F, W, S)
...
#if M68K_EMULATE_FC
extern uint           m68ki_access_pc;
extern uint           m68ki_access_address;
extern uint8          m68ki_access_fc;
extern uint8          m68ki_access_write;
extern uint8          m68ki_access_size;

extern uint           m68ki_fault_pending;
extern uint           m68ki_fault_pc;
extern uint           m68ki_fault_address;
extern uint8          m68ki_fault_fc;
...
INLINE uint m68ki_read_imm_16(void)
{
	...
	m68ki_remember_access(REG_PPC, CPU_PREF_ADDR, FLAG_S | FUNCTION_CODE_USER_PROGRAM, 0, 2);
...
#else
	m68ki_remember_access(REG_PPC, REG_PC, FLAG_S | FUNCTION_CODE_USER_PROGRAM, 0, 2);
	REG_PC += 2;
...
INLINE uint m68ki_read_imm_32(void)
{
	...
	m68ki_set_fc(FLAG_S | FUNCTION_CODE_USER_PROGRAM); /* auto-disable (see m68kcpu.h) */
	m68ki_check_address_error(REG_PC); /* auto-disable (see m68kcpu.h) */
	m68ki_remember_access(REG_PPC, CPU_PREF_ADDR, FLAG_S | FUNCTION_CODE_USER_PROGRAM, 0, 4);
	if(MASK_OUT_BELOW_2(REG_PC) != CPU_PREF_ADDR)
	{
		CPU_PREF_ADDR = MASK_OUT_BELOW_2(REG_PC);
		CPU_PREF_DATA = m68k_read_immediate_32(ADDRESS_68K(CPU_PREF_ADDR));
	...
	m68ki_set_fc(FLAG_S | FUNCTION_CODE_USER_PROGRAM); /* auto-disable (see m68kcpu.h) */
	m68ki_check_address_error(REG_PC); /* auto-disable (see m68kcpu.h) */
	m68ki_remember_access(REG_PPC, REG_PC, FLAG_S | FUNCTION_CODE_USER_PROGRAM, 0, 4);
	REG_PC += 4;
...
INLINE uint m68ki_read_8_fc(uint address, uint fc)
{
	...
	m68ki_remember_access(REG_PPC, address, fc, 0, 1);
...
INLINE uint m68ki_read_16_fc(uint address, uint fc)
{
	...
	m68ki_remember_access(REG_PPC, address, fc, 0, 2);
...
INLINE uint m68ki_read_32_fc(uint address, uint fc)
{
	m68ki_set_fc(fc); /* auto-disable (see m68kcpu.h) */
	m68ki_check_address_error(address); /* auto-disable (see m68kcpu.h) */
	m68ki_remember_access(REG_PPC, address, fc, 0, 4);
...
{
	m68ki_set_fc(fc); /* auto-disable (see m68kcpu.h) */
	m68ki_remember_access(REG_PPC, address, fc, 1, 1);
...
INLINE void m68ki_write_16_fc(uint address, uint fc, uint value)
{
	m68ki_set_fc(fc); /* auto-disable (see m68kcpu.h) */
	m68ki_check_address_error(address); /* auto-disable (see m68kcpu.h) */
	m68ki_remember_access(REG_PPC, address, fc, 1, 2);
	if (FAULT_NOT_PENDING) {
...
INLINE void m68ki_write_32_fc(uint address, uint fc, uint value)
{
	m68ki_set_fc(fc); /* auto-disable (see m68kcpu.h) */
	m68ki_check_address_error(address); /* auto-disable (see m68kcpu.h) */
	m68ki_remember_access(REG_PPC, address, fc, 1, 4);
	if (FAULT_NOT_PENDING) {
...
{
	int vector = EXCEPTION_BUS_ERROR;
	uint sr = m68ki_init_exception();

	uint address =  m68ki_fault_address;
	uint size = m68ki_fault_size;
	uint write = m68ki_fault_write;
	uint fc = m68ki_fault_fc;
...
#if 0
	printf("xxx: irq, vector 0x%x, old pc 0x%x, new pc 0x%x\n", vector, REG_PC, new_pc);
#endif
