...
static void build_opcode_table(void)
{
	...
	for(i=0;i<0x10000;i++)
	{
		...
		opcode = i;
 if (opcode == 0x0c80) printf("opcode 0c80\n");
		...
		for(ostruct = g_opcode_info;ostruct->opcode_handler != 0;ostruct++)
		{
			...
			{
 if (opcode == 0x0c80) printf("opcode 0c80! mask %x, valid %d\n", ostruct->ea_mask, valid_ea(opcode, ostruct->ea_mask));
				...
				{
if (i == 0x0c80) printf("opcode 0c80!! set table %x\n", i);
