...
uint8 m68ki_access_fc;
uint8 m68ki_access_write;
uint8 m68ki_access_size;

uint  m68ki_fault_pending;
uint  m68ki_fault_pc;
uint  m68ki_fault_address;
uint8 m68ki_fault_fc;
uint8 m68ki_fault_write;
uint8 m68ki_fault_size;
uint  m68ki_fault_regs[16];
uint  m68ki_fault_sr;
#endif
...
void m68k_mark_buserr(void)
{
	...
	m68ki_fault_pending = 1;
	m68ki_fault_pc = m68ki_access_pc;
	m68ki_fault_address = m68ki_access_address;
	m68ki_fault_fc = m68ki_access_fc;
	m68ki_fault_write = m68ki_access_write;
	m68ki_fault_size = m68ki_access_size;

	/* save state */
	for (i = 0; i < 16; i++) {
		m68ki_fault_regs[i] = cpu->dar[i];
	}
