...
M68KMAKE_OP(pack, 16, mm, ax7)
{
	if(CPU_TYPE_IS_EC020_PLUS(CPU_TYPE))
	{
		...
		uint ea_src = EA_AY_PD_8(); //xxx
		uint src = m68ki_read_8(ea_src);
		ea_src = EA_AY_PD_8(); //xxx
		src = ((src << 8) | m68ki_read_8(ea_src)) + OPER_I_16();

		m68ki_write_8(EA_A7_PD_8(), ((src >> 4) & 0x00f0) | (src & 0x000f)); //xxx
		return;
	}
	m68ki_exception_illegal();
}
...
M68KMAKE_OP(pack, 16, mm, ay7)
{
	...
	{
		/* Note: AX and AY are reversed in Motorola's docs */
		uint ea_src = EA_A7_PD_8(); //xxx
		uint src = m68ki_read_8(ea_src);
		ea_src = EA_A7_PD_8(); //xxx
		src = ((src << 8) | m68ki_read_8(ea_src)) + OPER_I_16();

		m68ki_write_8(EA_AX_PD_8(), ((src >> 4) & 0x00f0) | (src & 0x000f)); //xxx
		return;
	}
	m68ki_exception_illegal();
}
	...
	if(FLAG_S)
	{
		...
		if(CPU_TYPE_IS_010(CPU_TYPE))
		{
			...
			if (format_word == 8)
			{
				...
				new_pc = m68ki_pull_32();
				m68ki_fake_pull_16();	/* format word */
				spec_state = m68ki_pull_16();	/* special state */
				m68ki_pull_32();	/* fault address */

				m68ki_fake_pull_16();	/* reserved */
				m68ki_fake_pull_16();   /* data output buffer */
				m68ki_fake_pull_16();   /* reserved */
				m68ki_fake_pull_16();   /* data input buffer */
				m68ki_fake_pull_16();   /* reserved */
				m68ki_fake_pull_16();	/* ins input buffer */

				m68ki_fake_pull_32();	/* 16 words */
				m68ki_fake_pull_32();
				m68ki_fake_pull_32();
				m68ki_fake_pull_32();
				m68ki_fake_pull_32();
